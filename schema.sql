...
CREATE TABLE SupplierProduct (
    supplier_id INTEGER NOT NULL,
    product_id INTEGER NOT NULL,
    supply_price REAL NOT NULL CHECK (supply_price > 0),
...
CREATE TABLE PurchaseOrder (
    ...
    status TEXT NOT NULL CHECK (status IN ('ORDERED', 'PARTIAL', 'FULFILLED', 'CANCELLED')),
    store_id INTEGER NOT NULL,
    supplier_id INTEGER NOT NULL,
    FOREIGN KEY (store_id) REFERENCES Store(store_id),
    FOREIGN KEY (supplier_id) REFERENCES Supplier(supplier_id)
...
CREATE INDEX idx_supplierbrand_brand ON SupplierBrand(brand_id);
...
CREATE INDEX idx_purchaseorder_store ON PurchaseOrder(store_id);
...
CREATE INDEX idx_stockreceipt_po ON StockReceipt(po_id);
